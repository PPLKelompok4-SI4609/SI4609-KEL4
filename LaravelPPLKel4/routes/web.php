...
// routes/web.php
// ... (kode route kamu yang sudah ada) ...

// ROUTE KHUSUS UNTUK TESTING DUSK (HAPUS SAAT DEPLOY KE PRODUCTION!)
if (app()->environment('local', 'testing')) {
    Route::get('/dusk/login-and-set-session', function () {
        if (auth()->check()) {
            session(['two_factor_verified' => true]);
            return response('Session set!', 200);
        }
        return response('Not authenticated!', 401);
    })->middleware('auth')->name('dusk.set-session');
}