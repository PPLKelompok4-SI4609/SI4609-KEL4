<?php $__env->startSection('title', 'FloodRescue | Informasi Cuaca'); ?>
...
<main>
    ...
    <section id="hero" style="background: <?php echo e($bgGradient); ?>;">
        <img
        ...
        <div class="content text-white max-w-4xl mx-auto">
            ...
            <p class="text-sm sm:text-base font-semibold mb-6 drop-shadow-md" style="font-weight:600;">
                <?php echo e(\Carbon\Carbon::now()->translatedFormat('l, d M')); ?> • Update As Of
                <?php echo e(\Carbon\Carbon::now()->format('h:i A')); ?>

            </p>
            <img
                alt="<?php echo e($currentWeatherData['weather'][0]['description'] ?? 'weather icon'); ?>"
                class="mx-auto w-48 h-48 drop-shadow-xl"
                height="192"
                src="https://openweathermap.org/img/wn/<?php echo e($currentWeatherData['weather'][0]['icon'] ?? '01d'); ?>@4x.png"
                title="<?php echo e($currentWeatherData['weather'][0]['description'] ?? 'weather icon'); ?>"
                width="192"
            />
            <p class="text-5xl font-extrabold mt-4 drop-shadow-lg" style="font-weight:700;">
                <?php echo e(round($currentWeatherData['main']['temp'] ?? 0)); ?>

                <span class="text-4xl">°C</span>
            </p>
            <p class="text-xl font-semibold mt-2 drop-shadow-md" style="font-weight:600;">
                <?php echo e(ucfirst($currentWeatherData['weather'][0]['description'] ?? '')); ?>
            ...
            <div
                ...
                <div class="flex flex-col items-center">
                    ...
                    <span>Wind</span>
                    <span><?php echo e($currentWeatherData['wind']['speed'] ?? '-'); ?> m/s</span>
                </div>
                <div class="flex flex-col items-center">
                    <i aria-hidden="true" class="fas fa-tint text-xl mb-1"></i>
                    <span>Humidity</span>
                    <span><?php echo e($currentWeatherData['main']['humidity'] ?? '-'); ?>%</span>
                </div>
                <div class="flex flex-col items-center">
                    <i aria-hidden="true" class="fas fa-cloud-rain text-xl mb-1"></i>
                    <span>Rain</span>
                    <span><?php echo e($currentWeatherData['rain']['1h'] ?? ($currentWeatherData['rain']['3h'] ?? 0)); ?> mm</span>
                </div>
                <div class="flex flex-col items-center">
                    <i aria-hidden="true" class="fas fa-temperature-high text-xl mb-1"></i>
                    <span>Feels Like</span>
                    <span><?php echo e(round($currentWeatherData['main']['feels_like'] ?? 0)); ?>°C</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <section aria-label="Forecast and Flood Risk" id="forecast-flood-container">
        <?php if($dailyForecasts && $weeklyForecasts): ?>
            ...
            <div aria-labelledby="btn-daily" class="forecast-container" id="daily" role="tabpanel">
                <div class="forecast">
                    <?php $__currentLoopData = $dailyForecasts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="forecast-day" tabindex="0">
                            <h5><?php echo e(\Carbon\Carbon::parse($day['dt_txt'])->translatedFormat('l, d M')); ?></h5>
                            <p><?php echo e(\Carbon\Carbon::parse($day['dt_txt'])->format('h:i A')); ?></p>
                            <img
                                alt="<?php echo e($day['weather'][0]['description']); ?>"
                                class="mx-auto mb-2 w-14 h-14"
                                height="60"
                                loading="lazy"
                                src="https://openweathermap.org/img/wn/<?php echo e($day['weather'][0]['icon']); ?>@2x.png"
                                title="<?php echo e($day['weather'][0]['description']); ?>"
                                width="60"
                            />
                            <p><?php echo e(ucfirst($day['weather'][0]['description'])); ?></p>
                            <p class="temp"><?php echo e(round($day['main']['temp'])); ?>°C</p>
                            <p>
                                Rain: <?php echo e(isset($day['rain']['3h']) ? $day['rain']['3h'] . ' mm' : 'No rain'); ?>
            ...
            <div aria-labelledby="btn-weekly" class="forecast-container hidden" id="weekly" role="tabpanel">
                <div class="forecast">
                    <?php $__currentLoopData = $weeklyForecasts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $week): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="forecast-day" tabindex="0">
                            <h5><?php echo e($week['date']); ?></h5>
                            <img
                                alt="<?php echo e($week['weather']['description']); ?>"
                                class="mx-auto mb-2 w-14 h-14"
                                height="60"
                                loading="lazy"
                                src="https://openweathermap.org/img/wn/<?php echo e($week['weather']['icon']); ?>@2x.png"
                                title="<?php echo e($week['weather']['description']); ?>"
                                width="60"
                            />
                            <p><?php echo e(ucfirst($week['weather']['description'])); ?></p>
                            <p class="temp"><?php echo e(round($week['temp'])); ?>°C</p>
                            <p>Total Rainfall: <?php echo e($week['rain']); ?> mm</p>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
            ...
            <?php if(isset($floodRisk)): ?>
                <section aria-label="Flood Risk Prediction" id="flood-risk" role="region" class="<?php if($floodRisk === 'High'): ?> high <?php elseif($floodRisk === 'Medium'): ?> medium <?php else: ?> low <?php endif; ?>">
                    <h2>
                        Flood Risk Prediction
                        <?php if($floodRisk === 'High'): ?>
                            <i class="fas fa-exclamation-triangle text-red-600 ml-2" aria-hidden="true"></i>
                        <?php elseif($floodRisk === 'Medium'): ?>
                    ...
                    <p>
                        Flood Risk in <strong><?php echo e($city); ?></strong>: <span style="font-weight:700; color:#22c55e;"><?php echo e($floodRisk); ?></span>
                    </p>
                    <p>
                        Rainfall Level:
                        <?php echo e($floodRisk === "High" ? "≥ 100mm" : ($floodRisk === "Medium" ? "50-99mm" : "< 50mm")); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Asus\Documents\GitHub\SI4609-KEL4\LaravelPPLKel4\resources\views//cuaca/index.blade.php ENDPATH**/ ?>